...
- Code assistants (for dependency tracing): GitHub Copilot, Sourcegraph Cody, Cursor, JetBrains AI (or equivalents).
...
- Notes and reporting: Markdown, Confluence, spreadsheets (for the impact map).
...
- **AI misses dependencies**: the AI may not trace all call chains, especially for dynamic dispatch, runtime configuration, or reflection-based patterns. Mitigation: engineer validates the trace against their system knowledge; cross-reference with IDE dependency tools.
- **Overestimating blast radius**: the AI may flag every transitive dependency as impacted, making the map overwhelming. Mitigation: categorise impacts as direct, likely, and possible; focus review on direct and likely.
- **False sense of completeness**: the map may lead the team to believe all impacts are known when some are missed. Mitigation: frame the map as "best available analysis" and include confidence notes. Flag areas where the AI could not trace further.
- **Stale architecture context**: if the Architecture Summary or dependency map is outdated, the impact analysis may be inaccurate. Mitigation: verify critical dependencies directly in the code.

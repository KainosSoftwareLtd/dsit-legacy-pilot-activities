---
name: behaviour-baseline-characterisation-testing
description: "Use when establishing executable proof of current system behaviour before migration or refactoring. Derives tests from externally observable product feature specifications, not from implementation internals. Tests must remain valid as migration acceptance criteria for the target implementation. Use when: baselining legacy system behaviour, capturing API or route contracts before a rewrite, creating migration acceptance criteria."
...
## Inputs
- Product features specification (`.github/migrations/<migration-id>/discover/product-features.md` — produced by Legacy System Analyst skill). This is the **primary source** for test case derivation. Every test must trace to at least one entry here.
- Behaviour catalogue (`.github/migrations/<migration-id>/discover/behaviour-catalogue.md` — produced by Legacy System Analyst skill). Secondary source for technical detail (API methods, URLs, payload shapes, event contracts) needed to write concrete assertions.
...
## Outputs
- New automated characterisation tests.
...
- `.github/migrations/<migration-id>/test/baseline-summary.md`
...
5. Record confidence and gaps.
   - Summarise behaviours now covered, deferred items, implementation-coupled deviations, preference deviations, and residual risk.
...
Write `.github/migrations/<migration-id>/test/baseline-summary.md` as the final step, after CI verification is complete. This is the compressed brief the orchestrator loads instead of re-reading the full baseline evidence.

```markdown
# Baseline Summary: <project name>

**Phase completed:** <date>
**Migration ID:** <id>
**Full artefact path:** .github/migrations/<id>/test/baseline-evidence.md

## Coverage
- **Features covered:** <count> of <total> features from product-features.md
- **Strategy level breakdown:**
  - Level 1 (API contract): <count> features
  - Level 2 (Route/navigation): <count> features
  - Level 3 (Golden-master): <count> features
  - Level 4 (Integration): <count> features
  - Level 5 (Implementation-coupled — deviation): <count> features

## CI Verification
- **Status:** <all green | partial — <n> failures pre-existing | blocked>
- **Commands run:** ...

## Deferred Items
- **Count:** ...
- **Highest-risk deferred:** <feature ID and reason, one line each>

## Implementation-Coupled Deviations
<"None" or count and highest-risk item with migration impact>

## Preference Deviations
<"None" or count and items>

## Next Phase Input
The Migration Planner skill should read:
- This summary for coverage and risk context
- `baseline-evidence.md` only if it needs specific deferred item details
- `target-summary.md` for preferences and slice design constraints
```
...
artefacts_created_or_updated:
  - .github/migrations/<id>/test/baseline-evidence.md
  - .github/migrations/<id>/test/baseline-summary.md
  - <any new or modified test files>
blockers_or_waiting_on_human: <none | description>
next_action: <advance to Planning phase | resolve blocker>
```

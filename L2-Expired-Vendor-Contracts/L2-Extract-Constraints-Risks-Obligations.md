...
- **Missed obligations due to scattered provisions**: key obligations may be split across the main body, schedules, and amendments. Mitigation: process all document parts, not just the main agreement; prompt the AI to check for overriding provisions in amendments.
- **Misclassification of severity**: AI may under- or over-rate the impact of a constraint. Mitigation: reviewer validates all high-severity items and spot-checks medium items.
- **Ambiguous clauses reported as definitive**: AI may present uncertain interpretations as facts. Mitigation: require the AI to flag confidence level for each extraction; mark ambiguous items for legal review.

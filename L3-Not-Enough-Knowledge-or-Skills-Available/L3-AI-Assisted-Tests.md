...
- Code assistant (for test generation): GitHub Copilot, Sourcegraph Cody, Cursor, JetBrains AI (or equivalents).
- Test frameworks: Jest, Mocha, pytest, JUnit, NUnit (whatever the project uses).
- Coverage tools: Istanbul/nyc, JaCoCo, coverage.py, dotnet-coverage (or equivalents).
...
- **AI-generated tests that pass but test nothing useful**: tests may assert trivially true conditions or mock everything away. Mitigation: reviewer checks that assertions are meaningful and test real behaviour.
- **Tests that encode current bugs as expected behaviour**: the AI generates tests based on what the code does, not what it should do. Mitigation: reviewer validates expected values against the intended behaviour; flag any "tests passing on buggy code" cases.
- **Flaky tests**: AI-generated tests may have timing dependencies or order-of-execution issues. Mitigation: run tests multiple times before committing; remove or fix flaky tests.
- **Scope creep into production bug fixes**: discovering bugs during testing may tempt the team to fix them immediately. Mitigation: log bugs and defer fixes; this activity is about adding tests, not fixing production code.

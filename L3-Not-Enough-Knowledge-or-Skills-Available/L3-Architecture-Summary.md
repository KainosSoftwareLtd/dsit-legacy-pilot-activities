...
- AI code reasoning: an enterprise LLM with repository access (e.g. GitHub Copilot Chat, Sourcegraph Cody, or equivalent).
...
- **Incorrect component boundaries**: AI may group code incorrectly or merge distinct services into one. Mitigation: SME validates the component list.
- **Missing external integrations**: AI may not detect integrations configured outside the repo (e.g. environment variables, external config). Mitigation: ask the SME to confirm the integration list.
- **Diagram complexity**: auto-generated diagrams may be too detailed or too abstract. Mitigation: aim for 5-10 boxes; simplify with the SME.
- **Hallucinated interaction patterns**: AI may infer flows that do not exist. Mitigation: require source-file citations for each interaction described.
